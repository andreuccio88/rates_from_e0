...
%\section{Appendix: Prediction Interval}
%\label{Drop Out}	
%\subsection{DNN Drop Out: Uncertainty Estimation}
	
%Due to their complex structure, DNNs work memorizing the peculiar patterns in the data, this might lead to the "overfitting curse" in which the model catches the systematic component of data structure, ignoring the randomness.
%Regularization techniques allow for a better generalization of unobserved data points. Among the most widespread techniques such as Data augmentation, Earlystop, we deal with the so-called Dropout. Indeed we aimed to apply the dropout, not as a common regularization technique, but rather as a novel implementation useful to compute the uncertainty of the prediction.
%\paragraph{DNN Drop Out}
%From a generic DNN, considering the last layer that is the one who provides the output estimation. Dropout applied to this unit can be formalized as follow \cite{srivastava}: 
	
%   \begin{equation}
%   \hat{y}_{_D}=\phi_g(W_g^T\delta_{g-1}\textbf{outH}_{g-1}  +\textbf{b}_g), \nonumber
  % \end{equation}
  %  Where the subscript $D$ indicates that the dropout regularization has been applied, and
   
  % \begin{equation}
  % \delta \sim Bernoulli(p)
  % \end{equation}
  % This means $	delta$ is equal to $1$ with probability $p$ and $0$ otherwise.
%	For seek of simplicity let consider the simplified case of one linear layer, $n$ input vector $I$, nevertheless, the concept behind holds for the NN with nonlinear filter.
%	Minimizing loss function for model estimation
	
  %  \begin{equation}
%	E_D = \frac{1}{2}\left ( y-\sum_{i=1}^{n}\delta_iw_i I_i \right )^2
%	\label{eq:8}
  %  \end{equation}
    
  % We also consider the error function $E_{ens}$ associated with the ensemble of all possible subnetworks. Following \cite{baldia} it is easy to prove that the expectation of the gradient with Dropout, is equal to the gradient of Ensemble regular network $E_{ens}$ if $w’ = p \cdot w$.
        
  %  \begin{equation}
  %  E_{ens}= \frac{1}{2}\left ( y-\sum_{i=1}^{n}p_i \cdot w_i I_i \right )^2
  %  \label{eq:7}
  %  \end{equation}
    	
%	\begin{equation}
%	\frac{\partial E_D}{\partial w_i} = -y\delta_i I_i+w_i \delta_i^2I_i^2+ \sum_{j=1;j\neq i}^{n}w_i\delta_i\delta_j I_i I_j 
%	\label{eq:9}
%	\end{equation}
	
%	\begin{equation}
%		E\left[\frac{\partial E_{D}}{\partial w_{i}}\right] =\frac{\partial E_{ens}}{\partial w_{i}}+w_{i} p_{i}\left(1-p_{i}\right) I_{i}^{2}
%	\end{equation}
	
%	%\begin{equation}
%	%E_{R}=\frac{1}{2}\left(t-\sum_{i=1}^{n} p_{i} w_{i} I_{i}\right)^{2}+\sum_{i=1}^{n} %p_{i}\left(1-p_{i}\right) w_{i}^{2} I_{i}^{2}
%	%\end{equation}
%	This means minimizing the Dropout loss (in \ref{eq:8}) is equivalent to minimizing a regularized ensemble network, shown in Eq. 7 below.
	
%\paragraph{Uncertainty Estimation} We then propose a simple algorithm in order to obtain uncertainty estimation. Thus in the spirit of simulative models i.e. MCMC, we aim to apply the dropout, as a novel implementation in order to compute the uncertainty of estimation. 
%PLEASE, MAKE ATTENTION THIS ALGORITHM IS NOT A COMMON PRACTICE AMONG DEEP LEARNING PRACTITIONERS, IT IS A MY OWN PROPOSAL.}\\
	
%	\framebox{
%		\parbox[t][10.0cm]{15.0cm}{
%			\addvspace{0.2cm}
%		\small
			 		
%		\begin{itemize}
%				\item \textbf{Input:} Data($e_0_{train},m_x__{train}$; NN; X)
%				\item \textbf{Output:} Prediction Interval\\
%				 Where $D(x)={d(x_1);...;d(x_N)}$ is DropOut Simulations and  $x\in X\sim U(0,1)$ is DropOut Rate\\
%			\begin{enumerate}
%				\item \textbf{for} d=1 to N \textbf{do}\\
%				\item $Pred_{d_1} {NN[d(x_1)]}$ \\                             
						
%						. . .\\
   %                                          	. . .\\
   %                                     $Pred_{d_N} {NN[d(x_N)]}$ \\
%				\item $Q_{(0.95-0.05)}{Pred_D [NN_{D_X}]}$\\
%				Where $Pred_{D} NN[D_X]$ is Matrix of all prediction simulations
   %                             \textbf{end for}	
%			\end{enumerate}
%		\end{itemize} 
...
%\begin{figure}[H]
%		\centering
%		\includegraphics[width=0.8\linewidth]{IC_drop_RUS_F.jpeg}\\
%		\includegraphics[width=0.8\linewidth]{IC_drop_ITA_F.jpeg}
%\caption{I.C. Drop out}
		%	\label{fig:ktmale}
%\end{figure}
%\subsection{Bootstrap Life Expectancy: Uncertainty Estimation}
%to do...
...
%\bibitem[Baldi, P.,and Sadowski(2013)]{baldi}
...
%\bibitem[Bongaarts and Feeney(2002)]{BongFeen2002}
%Bongaarts, J., Feeney, G.(2002). How long do we live? Population and Development Review, 28(1):13-29.
%\bibitem[Bongaarts and Feeney(2003)]{BongFeen2003}
%Bongaarts, J., Feeney, G. (2003). Estimating mean lifetime. Proceedings of the National Academy of Sciences, 100(23):13127-13133.
...
%\bibitem[Camarda(2019)]{Camarda}
%Camarda, C.G. (2019). Smooth constrained mortality forecasting. Demographic Research, 41(38): 1091-1130.
...
%Nigri, A., Levantesi, S., Marino, M., Scognamiglio, S., Perla, F. (2019). A Deep Learning Integrated Lee–Carter Model. Risks 2019, 7, 33; doi:10.3390/risks7010033
...
%\bibitem[Srivastava et al.(2014)]{srivastava}
