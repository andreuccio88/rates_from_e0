...
D<- df_m %>%filter(year>=1950,year<=2014,age<=100,country==cnt)
...
LC <- matr.lc[i,c(1:101)]


p <- data.frame(age=0:100,NN=NN,true=true,LC=LC)

 p %>% ggplot(aes(age,true))+geom_point(size=1.7,alpha=0.8)+
  geom_line(aes(age,NN),color="red",size=1.7,alpha=1)+
  geom_line(aes(age,LC),color="black",size=1.7,alpha=1)+
...
dimnames(tab) <- list(c("NN","Smooth NN","LC"),c("MAE","RMSE"))
tab[,1] <-   c(mae(matr.val_predi,matr.val),  mae(matr.NN,matr.val),  mae(matr.lc,matr.val))
tab[,2] <-   c(rmse(matr.val_predi,matr.val),  rmse(matr.NN,matr.val),  rmse(matr.lc,matr.val))
tab
