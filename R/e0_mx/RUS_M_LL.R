...
D<- df_m %>%filter(year>=1959,year<=2014,age<=100,country==cnt)
...
LC.F<- fit(lc(link = "log"), Dxt = Dx[,1:47], Ext = Ex[,1:47], ages = , years = year,wxt = wxt)
...
D %>% filter(year==1959,country==cnt,age<=100) %>% select(log_mx,age) %>% ggplot(aes(age,log_mx))+geom_point()
...
B <- D %>%filter(year>=1959,year<=2005,age<=100,country==cnt) %>%  select(year,age,log_mx,ex)
