...
Autotuning_DNN <- function(cnt,age,ys,yf,yv,reg,un,ep,gender){
  
  year <- ys:yf; l <- length(year); last <- yf+15; un <- un;ep <- ep
  ...
  #
  ##############
  tuning = expand.grid(epochs =ep , unit = un)
  ...
  tuning$performance[g] = rmse(NN_s, Real)
